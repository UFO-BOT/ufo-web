      ...
      <v-card>
        ...
        <div class="item-settings">
          <v-form ref="form" v-model="valid">
            ...
            <v-select v-model="automod.punishment.type" :items="punishments" :disabled="!automod.enabled" :label="content.subtitles.punishment" class="punishment-select"></v-select>
            <v-textarea v-model="automod.punishment.reason" filled :disabled="!automod.enabled" counter="200"
                        :label="content.subtitles.reason" class="general-automod-field pa-0"></v-textarea>
            <div v-if="automod.punishment.type && punishments.find(p => p.value === automod.punishment.type).duration">
              ...
              <v-text-field v-model="automod.punishment.duration" :disabled="!automod.enabled" type="number" :label="content.subtitles.duration"
                            class="number-input mt-0"></v-text-field>
              <v-select v-model="automod.punishment.durationUnit" :disabled="!automod.enabled" :items="content.subtitles.units" :label="content.subtitles.unit"
                        class="unit-select mt-0"></v-select>
            </div>
            <div class="subtitle">{{ content.subtitles.whitelist }}</div>
            <div class="automod-flex">
              <div><v-select v-model="automod.whitelist.roles" :disabled="!automod.enabled" :items="roles" class="whitelist-select mt-0" :label="content.subtitles.roles" multiple chips deletable-chips></v-select></div>
              <div><v-select v-model="automod.whitelist.channels" :disabled="!automod.enabled" :items="channels" class="whitelist-select mt-0" :label="content.subtitles.channels" multiple chips deletable-chips></v-select></div>
            </div>
            <div class="subtitle">{{ content.subtitles.rest }}</div>
            <div v-if="type === 'invites'">
              <v-combobox v-model="automod.whitelistGuilds" :disabled="!automod.enabled" multiple chips deletable-chips :label="content.subtitles.allowedServers"></v-combobox>
            </div>
          </v-form>
        </div>
      </v-card>
...
export default {
  ...
  computed: {
    channels() {
      ...
      if (!guild) return [];
      return ParseForSelect.channels(guild.channels, {none: false, checkBotManageable: false, checkMemberManageable: false})
    },
    roles() {
      let guild = this.$store.getters.guilds.find(g => g.id === this.$route.params.id)
      if (!guild) return [];
